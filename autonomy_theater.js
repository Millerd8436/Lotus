...
class AutonomyTrap {
  ...
  async applyTimePressure(seconds, offerId) {
    ...
    if (this.ui?.showCountdown_Exploit)
      await this.ui.showCountdown_Exploit(
        this.session,
        this.config,
        seconds,
        offerId,
      );
    this.echo?.logAction("urgency_cue", {
  ...
  presentMisleadingTestimonial(testimonial, author) {
    ...
    this.autonomyViolations.push({
      ...
      description: `Fake testimonial: "${testimonial}" - ${author}`,
      severity: "medium",
  ...
  createSunkCostPressure(investment) {
    ...
    this.autonomyViolations.push({
      ...
      description: `Leveraging user's invested time/data: ${investment}`,
      severity: "medium",
      kantianViolation: "Exploits cognitive bias to prevent rational choice",
      timestamp: new Date().toISOString(),
    });
  ...
  applyCognitiveOverload(complexText) {
    ...
    this.autonomyViolations.push({
      ...
      description: `Excessive complexity: ${complexText.length} characters`,
      severity: "high",
  ...
  generateKantianSummary() {
    const deceptionCount = this.autonomyViolations.filter((v) =>
      v.kantianViolation.includes("Deception"),
    ).length;
    const autonomyCount = this.autonomyViolations.filter((v) =>
      v.kantianViolation.includes("autonomy"),
    ).length;
    const meansCount = this.autonomyViolations.filter((v) =>
    ...
    return {
      ...
      overallAssessment:
        deceptionCount + autonomyCount + meansCount > 0
  ...
  generateRestorationRecommendations() {
    ...
    if (this.trapTypes.timePressure)
      recs.push("Remove artificial time constraints");
    if (this.trapTypes.defaultBias)
      recs.push("Make all options explicitly opt-in");
    if (this.trapTypes.socialProof)
      recs.push("Provide only verified testimonials");
    if (this.trapTypes.scarcityIllusion)
      recs.push("Remove false scarcity claims");
    if (this.trapTypes.cognitiveOverload)
      recs.push("Simplify language and information");
