...
interface AutonomyTheaterMetric {
  pattern: string;
  description: string;
...
const Phase3EducationalReflection: React.FC = () => {
  ...
  const quizQuestions: QuizQuestion[] = [
    {
      ...
      correctAnswer: 1,
      explanation: "Confirmshaming uses guilt-inducing language like 'Only irresponsible borrowers skip this protection' to manipulate users into selecting unwanted add-ons.",
    ...
    },
    {
      ...
      correctAnswer: 2,
      explanation: "The exploitative example showed 664% APR, which is typical for predatory payday loans in states with weak usury laws.",
      category: "fees"
    },
    {
      id: "rights-1",
      question: "Under the Fair Debt Collection Practices Act, which is prohibited?",
      options: [
      ...
      correctAnswer: 1,
      explanation: "The FDCPA prohibits debt collectors from making threats they cannot or will not carry out, including false legal threats.",
      category: "rights"
    },
    {
      id: "alternatives-1",
      question: "What is the maximum APR for Credit Union Payday Alternative Loans (PALs)?",
      options: ["18%", "28%", "36%", "48%"],
      correctAnswer: 1,
      explanation: "Credit Union PALs have a maximum APR of 28%, making them much more affordable than traditional payday loans.",
  ...
  const autonomyTheaterMetrics = useMemo(() => [
    ...
    },
    {
      ...
      description: "Fake trust badges, false testimonials, manufactured credibility",
      severity: "High",
      frequency: 89,
  ...
  const calculateAutonomyScore = useCallback(() => {
    const totalViolations = autonomyTheaterMetrics.reduce((sum, metric) => {
      const severityWeight = metric.severity === 'High' ? 4 : 
                           metric.severity === 'Medium' ? 3 : 
                           metric.severity === 'Low' ? 2 : 1;
  ...
  const renderOverview = () => (
    <div className="space-y-8">
      <div className="text-center">
        <h2 className="text-4xl font-bold text-gray-800 mb-4">Educational Analysis & Reflection</h2>
        <p className="text-xl text-gray-600 max-w-4xl mx-auto">
          Compare the two lending experiences and understand the psychology behind predatory practices
        </p>
      </div>

      {/* Autonomy Theater Summary */}
      <div className="bg-red-50 border border-red-200 rounded-lg p-6">
        <h3 className="text-2xl font-bold text-red-800 mb-4">🎭 Autonomy Theater Detection</h3>
        <div className="grid md:grid-cols-2 gap-6">
          <div>
            <p className="text-red-700 mb-4">
              "Autonomy Theater" refers to the illusion of choice and control while actually constraining user decisions through manipulation.
            </p>
            <div className="space-y-2">
              <div className="flex justify-between">
                <span>Manipulation Patterns Detected:</span>
                <span className="font-bold">{autonomyTheaterMetrics.length}</span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-bold text-red-600">{autonomyScore}/100</span>
              </div>
            </div>
          </div>
          <div className="bg-white p-4 rounded-lg">
            <h4 className="font-bold text-gray-800 mb-2">Most Common Violations:</h4>
            <div className="space-y-2 text-sm">
              {autonomyTheaterMetrics.slice(0, 3).map((metric, index) => (
                <div key={index} className="flex items-center space-x-2">
                  <span className={`w-2 h-2 rounded-full ${
                    metric.severity === 'Critical' ? 'bg-red-600' :
                    metric.severity === 'High' ? 'bg-orange-500' : 'bg-yellow-500'
                  }`}></span>
      ...
      <div className="bg-white rounded-lg shadow-lg p-6">
        <h3 className="text-2xl font-bold text-gray-800 mb-6 text-center">Phase Comparison Analysis</h3>
        <div className="overflow-x-auto">
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-gray-50">
                <th className="border p-3 text-left">Category</th>
                <th className="border p-3 text-left bg-red-50">Phase 1: Exploitative</th>
                <th className="border p-3 text-left bg-green-50">Phase 2: Ethical</th>
            ...
            <tbody>
              {comparisonData.map((row, index) => (
                <tr key={index} className="hover:bg-gray-50">
                  ...
                  <td className="border p-3 text-sm bg-red-50">{row.exploitative}</td>
                  <td className="border p-3 text-sm bg-green-50">{row.ethical}</td>
  ...
  const renderAutonomyTheater = () => (
    <div className="space-y-8">
      <div className="text-center">
        <h2 className="text-4xl font-bold text-red-600 mb-4">🎭 Autonomy Theater Analysis</h2>
        <p className="text-xl text-gray-600 max-w-4xl mx-auto">
          How predatory lenders create the illusion of choice while manipulating your decisions
        </p>
      </div>
        ...
        {autonomyTheaterMetrics.map((metric, index) => (
          <div key={index} className="bg-white rounded-lg shadow-lg p-6 border-l-4 border-red-500">
            <div className="flex justify-between items-start mb-3">
              <h3 className="text-lg font-bold text-gray-800">{metric.pattern}</h3>
              <span className={`px-2 py-1 rounded text-xs font-bold ${
                metric.severity === 'Critical' ? 'bg-red-600 text-white' :
                metric.severity === 'High' ? 'bg-orange-500 text-white' :
                metric.severity === 'Medium' ? 'bg-yellow-500 text-white' : 'bg-gray-400 text-white'
              }`}>
                {metric.severity}
              </span>
            </div>
      ...
      <div className="bg-red-50 border border-red-200 rounded-lg p-8 text-center">
        <h3 className="text-2xl font-bold text-red-800 mb-4">Overall Autonomy Violation Score</h3>
        <div className="text-6xl font-black text-red-600 mb-4">{autonomyScore}/100</div>
        <p className="text-red-700 text-lg">
          This score represents the cumulative impact of manipulation tactics on user autonomy.
          Higher scores indicate more severe violations of user agency and informed consent.
        </p>
      </div>
  ...
  const renderQuiz = () => (
    <div className="space-y-8">
      <div className="text-center">
        <h2 className="text-4xl font-bold text-blue-600 mb-4">📝 Knowledge Assessment</h2>
        <p className="text-xl text-gray-600">Test your understanding of predatory lending practices</p>
      </div>
        ...
        <div className="bg-white rounded-lg shadow-lg p-8 max-w-3xl mx-auto">
          <div className="mb-6">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl font-bold text-gray-800">Question {currentQuiz + 1} of {quizQuestions.length}</h3>
              <div className="text-sm text-gray-600">
                {Math.round((currentQuiz / quizQuestions.length) * 100)}% Complete
              </div>
            </div>
            <div className="w-full bg-gray-200 rounded-full h-2">
              <div 
        ...
        <div className="bg-white rounded-lg shadow-lg p-8 max-w-3xl mx-auto">
              ...
              return (
                <div key={index} className={`p-4 rounded-lg border-2 ${isCorrect ? 'border-green-200 bg-green-50' : 'border-red-200 bg-red-50'}`}>
                  <div className="flex items-center mb-2">
                    <span className={`mr-3 ${isCorrect ? 'text-green-600' : 'text-red-600'}`}>
                      {isCorrect ? '✓' : '✗'}
                    </span>
          ...
          <div className="mt-8 text-center">
            <div className="text-3xl font-bold mb-4">
              Score: <span className={getScoreColor(Math.round((quizAnswers.filter((ans, i) => ans === quizQuestions[i].correctAnswer).length / quizQuestions.length) * 100))}>
                {Math.round((quizAnswers.filter((ans, i) => ans === quizQuestions[i].correctAnswer).length / quizQuestions.length) * 100)}%
              </span>
            </div>
  ...
  const renderResearch = () => (
    <div className="space-y-8">
      <div className="text-center">
        <h2 className="text-4xl font-bold text-purple-600 mb-4">📊 Research Findings</h2>
        <p className="text-xl text-gray-600">Real data about predatory lending practices</p>
      </div>

      <div className="grid md:grid-cols-2 gap-8">
        <div className="bg-white rounded-lg shadow-lg p-6">
          <h3 className="text-xl font-bold text-gray-800 mb-4">ACH Exploitation Statistics</h3>
          <div className="space-y-4">
            <div className="bg-red-50 p-4 rounded-lg">
              <div className="text-3xl font-bold text-red-600">32%</div>
              <div className="text-red-700">of borrowers suffer unauthorized ACH withdrawals</div>
            </div>
            <div className="bg-orange-50 p-4 rounded-lg">
              <div className="text-3xl font-bold text-orange-600">$35</div>
              <div className="text-orange-700">average NSF fee per failed attempt</div>
            </div>
            <div className="bg-yellow-50 p-4 rounded-lg">
              <div className="text-3xl font-bold text-yellow-600">5.7</div>
              <div className="text-yellow-700">average withdrawal attempts per failed payment</div>
            </div>
          </div>
        </div>

        <div className="bg-white rounded-lg shadow-lg p-6">
          <h3 className="text-xl font-bold text-gray-800 mb-4">Rollover Trap Statistics</h3>
          <div className="space-y-4">
            <div className="bg-red-50 p-4 rounded-lg">
              <div className="text-3xl font-bold text-red-600">80%</div>
              <div className="text-red-700">of payday loan revenue comes from borrowers trapped in debt cycles</div>
            </div>
            <div className="bg-orange-50 p-4 rounded-lg">
              <div className="text-3xl font-bold text-orange-600">10</div>
              <div className="text-orange-700">average number of rollovers per borrower per year</div>
            </div>
            <div className="bg-yellow-50 p-4 rounded-lg">
              <div className="text-3xl font-bold text-yellow-600">$520</div>
              <div className="text-yellow-700">average annual fees paid by chronic borrowers</div>
            </div>
          </div>
        </div>
      </div>
  ...
  // Main render logic
  switch (currentView) {
    case 'autonomy': return renderAutonomyTheater();
