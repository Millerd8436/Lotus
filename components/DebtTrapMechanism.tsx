...
const DebtTrapMechanism: React.FC = () => {
  ...
  const performRollover = () => {
    ...
    setLoanHistory(prev => prev.map(loan => 
      loan.id === currentLoan.id ? rolledLoan : loan
    ));
  ...
  const createMultipleLoanCycle = () => {
    ...
    updateStatistics({
      ...
      totalFeesPaid: statistics.totalFeesPaid + newLoans.reduce((sum, loan) => sum + loan.feesPaid, 0),
  ...
  return (
    <div className="debt-trap-mechanism" style={{ maxWidth: '900px', margin: '0 auto', padding: '1rem' }}>
      <div style={{
        ...
        <p style={{ margin: 0, fontSize: '1.1rem', opacity: 0.9 }}>
          Experience how payday lenders design business models around debt dependency
        </p>
      </div>
      ...
      <div style={{
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '1rem' }}>
          <div style={{ textAlign: 'center' }}>
            ...
            <div style={{ fontSize: '0.9rem', color: '#666' }}>Loans This Year</div>
          </div>
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: '2rem', fontWeight: 'bold', color: '#f44336' }}>
              ${statistics.totalFeesPaid}
            </div>
            <div style={{ fontSize: '0.9rem', color: '#666' }}>Total Fees Paid</div>
          </div>
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: '2rem', fontWeight: 'bold', color: '#4caf50' }}>
              ${statistics.totalPrincipalPaid}
            </div>
            <div style={{ fontSize: '0.9rem', color: '#666' }}>Principal Paid</div>
          </div>
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: '2rem', fontWeight: 'bold', color: '#ff9800' }}>
              {statistics.daysInDebt}
            </div>
            <div style={{ fontSize: '0.9rem', color: '#666' }}>Days in Debt</div>
          </div>
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: '2rem', fontWeight: 'bold', color: getProgressColor(statistics.dependencyScore) }}>
              {statistics.dependencyScore}/10
            </div>
            <div style={{ fontSize: '0.9rem', color: '#666' }}>Dependency Score</div>
          </div>
        </div>
      </div>
        ...
        <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
          <button
            ...
            style={{
              ...
              background: simulationActive ? '#ccc' : 'linear-gradient(90deg, #7b1fa2, #e91e63)',
              color: 'white',
              border: 'none',
              borderRadius: '8px',
              fontWeight: 'bold',
          ...
          >
            {simulationActive ? 'Simulating Debt Trap...' : 'Start Debt Trap Simulation'}
          </button>
          <button
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '1rem' }}>
          {achievements.map((achievement) => (
            <div key={achievement.id} style={{
              background: achievement.unlocked ? '#f3e5f5' : '#f8f9fa',
              padding: '1rem',
              borderRadius: '8px',
              border: achievement.unlocked ? '2px solid #7b1fa2' : '1px solid #e9ecef',
              opacity: achievement.unlocked ? 1 : 0.6
            }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.5rem' }}>
                <span style={{ fontSize: '1.5rem' }}>{achievement.icon}</span>
                <span style={{ fontWeight: 'bold', fontSize: '1rem' }}>{achievement.title}</span>
              ...
              <div style={{ fontSize: '0.8rem', marginBottom: '0.5rem' }}>
                <span style={{ color: '#4caf50', fontWeight: 'bold' }}>Marketed as:</span> {achievement.deceptiveName}
              </div>
              <div style={{ fontSize: '0.8rem', color: '#f44336' }}>
                <span style={{ fontWeight: 'bold' }}>Reality:</span> {achievement.realMeaning}
              </div>
              <div style={{ marginTop: '0.5rem' }}>
                <div style={{ fontSize: '0.8rem', color: '#666' }}>Harm Level:</div>
                <div style={{ display: 'flex', gap: '2px' }}>
                  {[...Array(5)].map((_, i) => (
                    <div key={i} style={{
                      ...
                      height: '12px',
                      borderRadius: '50%',
                      background: i < achievement.harmLevel ? '#f44336' : '#e0e0e0'
                    }} />
                  ))}
                </div>
              </div>
      ...
      <div style={{
        ...
        {loanHistory.length === 0 ? (
          <div style={{ textAlign: 'center', color: '#666', padding: '2rem' }}>
            No loan history yet. Click &quot;Start Debt Trap Simulation&quot; to see the progression.
          </div>
        ) : (
            ...
            {loanHistory.map((loan) => (
              <div key={loan.id} style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
                  ...
                  <span style={{
                    background: loan.status === 'active' ? '#4caf50' : 
                              loan.status === 'rolled_over' ? '#ff9800' : '#6c757d',
                    color: 'white',
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: '0.5rem', fontSize: '0.9rem' }}>
                  <div><strong>Principal:</strong> ${loan.principal}</div>
                  <div><strong>Fees:</strong> ${loan.fees}</div>
                  <div><strong>Total Due:</strong> ${loan.totalDue}</div>
                  <div><strong>Rollovers:</strong> {loan.rolloverCount}</div>
                </div>
                {loan.rolloverCount > 0 && (
                  <div style={{
                    ...
                    🔄 Rolled over {loan.rolloverCount} time(s) - Principal never reduced!
                  </div>
                )}
              </div>
            ))}
      ...
      <div style={{
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '1rem' }}>
          <div style={{ background: '#ffebee', padding: '1rem', borderRadius: '8px', border: '1px solid #ffcdd2' }}>
            <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: '#d32f2f' }}>80%</div>
          ...
          <div style={{ background: '#fff3cd', padding: '1rem', borderRadius: '8px', border: '1px solid #ffe0a6' }}>
            <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: '#856404' }}>15%</div>
            <div style={{ fontSize: '0.9rem', color: '#666' }}>
          ...
          <div style={{ background: '#f3e5f5', padding: '1rem', borderRadius: '8px', border: '1px solid #e1bee7' }}>
            <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: '#7b1fa2' }}>75%</div>
            <div style={{ fontSize: '0.9rem', color: '#666' }}>
          ...
          <div style={{ background: '#e8f5e8', padding: '1rem', borderRadius: '8px', border: '1px solid #c8e6c9' }}>
            <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: '#2e7d32' }}>$0</div>
            <div style={{ fontSize: '0.9rem', color: '#666' }}>
      ...
      <div style={{
        ...
        <h3 style={{ margin: '0 0 1rem 0', fontSize: '1.3rem', fontWeight: 'bold', color: '#2e7d32' }}>
          🎓 Educational: The Debt Trap Business Model
        </h3>
        <div style={{ fontSize: '1rem', lineHeight: '1.6', color: '#333' }}>
          <p><strong>The 80/15 Rule:</strong> 80% of loans are rolled over within 2 weeks, while only 15% are paid off on time without taking a new loan.</p>
          <p><strong>Revenue Concentration:</strong> 75% of lender revenue comes from borrowers trapped in 10+ loans per year - this is the target customer.</p>
          <p><strong>Rollover Trap:</strong> When you &quot;rollover&quot; a loan, you only pay the fee (typically $50) but the principal amount stays the same. This creates a cycle where you pay repeatedly without reducing debt.</p>
          <p><strong>Gamification Deception:</strong> Lenders use &quot;achievements&quot; and &quot;loyalty programs&quot; to make debt dependency feel positive and rewarding.</p>
          <p><strong>Business Model:</strong> The industry is designed around borrowers who cannot pay back the loan quickly - these are the most profitable customers.</p>
          <p><strong>Rollover &ldquo;Option&rdquo;:</strong> When borrowers can&apos;t repay in full, lenders offer to &ldquo;roll over&rdquo; the loan for an additional fee.</p>
          <p>Lenders make borrowers feel &ldquo;grateful&rdquo; for rollover &ldquo;help&rdquo; while trapping them deeper in debt.</p>
          <p><strong>&ldquo;Debt Trap&rdquo; Mechanics:</strong> Average borrower pays more in fees ($520) than original loan amount ($375).</p>
          <p><strong>Industry Research:</strong> &ldquo;Debt Trap&rdquo; by Design - Lenders make 76% of revenue from borrowers trapped in 10+ loans per year.</p>
          <p><strong>Rollover Economics:</strong> &ldquo;Convenience&rdquo; fees of $50+ generate $4.2 billion annually while borrowers sink deeper into debt.</p>
          <p><strong>Psychological Manipulation:</strong> Rollover &ldquo;offers&rdquo; framed as &ldquo;help&rdquo; and &ldquo;flexibility&rdquo; when they&apos;re designed to create dependency.</p>
          <p>Research: Average payday borrower spends 5 months per year in debt, paying $520 in fees for $375 in credit - proving the &ldquo;short-term&rdquo; loan is actually a &ldquo;long-term debt trap&rdquo;.</p>
        </div>
      </div>
    </div>
  );
