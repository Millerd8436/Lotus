...
export const EducationProvider: React.FC<EducationProviderProps> = ({
  ...
  >([
    {
      ...
      content:
        "Dark patterns are user interface design choices that benefit the business at the expense of the user. They exploit cognitive biases and psychological vulnerabilities to manipulate user behavior.",
      type: "lesson",
      difficulty: "beginner",
    ...
    },
    {
      ...
      content:
        "Payday lenders use various tactics including hidden fees, artificial urgency, and targeting vulnerable populations. Understanding these tactics helps protect against exploitation.",
      type: "lesson",
    ...
    },
    {
      ...
      content:
        "Our brains are wired with cognitive biases that can lead to poor financial decisions. Recognizing these biases is the first step toward making better choices.",
      type: "lesson",
      difficulty: "intermediate",
    ...
    },
    {
      ...
      content:
        "Understanding your rights under consumer protection laws and state regulations can help you avoid predatory lending practices.",
      type: "lesson",
      difficulty: "beginner",
    ...
    },
    {
      ...
      content:
        "Test your ability to identify common dark patterns used in financial services and other industries.",
      type: "quiz",
      difficulty: "intermediate",
    ...
    },
    {
      ...
      content:
        "Follow the story of a fictional character who falls into a payday loan debt trap and learn how to avoid similar situations.",
      type: "case_study",
      difficulty: "advanced",
  ...
  const getEducationalInsights = (): string[] => {
    ...
    if (completedModules.length === 0) {
      insights.push(
        "Start with the basic modules to build your understanding of dark patterns"
      );
      return insights;
    }
    ...
    if (avgScore < 70) {
      insights.push(
        "Your comprehension score suggests you might benefit from reviewing some concepts"
      );
    }

    if (completedModules.length >= 3) {
      insights.push(
        "You've completed several modules - consider taking the advanced case studies"
      );
    }

    const darkPatternModules = completedModules.filter((m) =>
      m.tags.includes("dark-patterns")
    );
    if (darkPatternModules.length > 0) {
      insights.push(
        "You've learned about dark patterns - practice identifying them in real-world scenarios"
      );
    }
  ...
  const getRecommendations = (): string[] => {
    ...
    if (!completedIds.includes("dark-patterns-101")) {
      recommendations.push(
        "Start with 'Understanding Dark Patterns' to build foundational knowledge"
      );
    }

    if (
      completedIds.includes("dark-patterns-101") &&
      !completedIds.includes("payday-loan-tactics")
    ) {
      recommendations.push(
        "Learn about 'Common Payday Loan Tactics' to understand specific exploitation methods"
      );
    }

    if (
      completedIds.includes("payday-loan-tactics") &&
      !completedIds.includes("cognitive-biases")
    ) {
      recommendations.push(
        "Study 'Cognitive Biases in Financial Decisions' to understand psychological vulnerabilities"
      );
    }

    if (
      completedIds.includes("cognitive-biases") &&
      !completedIds.includes("dark-pattern-quiz")
    ) {
      recommendations.push(
        "Test your knowledge with the 'Dark Pattern Recognition Quiz'"
      );
    }

    if (
      completedIds.includes("dark-pattern-quiz") &&
      !completedIds.includes("case-study-1")
    ) {
      recommendations.push(
        "Apply your learning with 'Case Study: The Debt Trap'"
      );
    }

    if (completedModules.length >= 4) {
      recommendations.push(
        "Consider exploring additional resources on consumer protection and financial literacy"
      );
    }
...
const EducationalQuiz: React.FC<{
  ...
  const questions = [
    ...
    {
      question:
        "What cognitive bias makes people more likely to choose immediate rewards over long-term benefits?",
      options: [
        "Confirmation bias",
        "Present bias",
        "Anchoring bias",
