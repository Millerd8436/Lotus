...
import { AutonomyViolation, LotusSession, UserChoice } from "../types/lotus.js";
import { calculateAPR, formatCurrency } from "../utils.js";
import {
  BehavioralTracker,
  CountdownTimer,
  FakeAvailability,
...
// Import comprehensive system orchestrator
...
import {
  ...
  createPhaseAutonomyTheater,
} from "../lib/autonomy-theater";
...
const COMPREHENSIVE_STATE_REGULATIONS: { [key: string]: StateRegulation } = {
  ...
  NV: {
    ...
    consumerProtections: ["Basic disclosure requirements"],
...
const ADVANCED_TRAP_MECHANISMS: AdvancedTrapMechanism[] = [
  {
    ...
    description:
      "Manipulate perception of future consequences vs immediate needs",
    implementation: () => console.log("Implementing temporal discounting trap"),
    psychological_basis:
      "Humans heavily discount future costs when under stress",
    effectiveness: 87,
  ...
  },
  {
    ...
    description:
      "Present inflated reference points to make predatory terms seem reasonable",
...
const EDUCATIONAL_MODULES: EducationalModule[] = [
  {
    ...
    interactiveElements: [
      {
        type: "simulation",
        data: {
          scenarios: ["urgency_pressure", "hidden_fees", "rollover_trap"],
        },
    ...
    assessments: [
      {
        question:
          "What is the most effective way to resist urgency pressure tactics?",
        options: [
        ...
        correct: 1,
        explanation:
          "Legitimate financial products should not require immediate decisions. Taking time to research and consider alternatives is your right and the best protection against manipulation.",
        difficulty: 2,
      },
    ],
    realWorldExamples: [
...
export default function LotusSimulator({
  phase,
  onPhaseComplete,
}: LotusSimulatorProps) {
  // Core state management
  const [currentPhase, setCurrentPhase] = useState(phase);
  const [exploitativeSession, setExploitativeSession] =
    useState<LoanSession | null>(null);
  const [ethicalSession, setEthicalSession] = useState<LoanSession | null>(
    null
  );
  const [userChoices, setUserChoices] = useState<UserChoice[]>([]);
  const [autonomyViolations, setAutonomyViolations] = useState<
    AutonomyViolation[]
  >([]);
  ...
  // --- COMPREHENSIVE SYSTEM ORCHESTRATOR STATE ---
  const [comprehensiveOrchestrator, setComprehensiveOrchestrator] =
    useState<any>(null);
  const [ethicsEngine, setEthicsEngine] = useState<any>(null);
  const [behavioralEngine, setBehavioralEngine] = useState<any>(null);
  const [darkPatternEngine, setDarkPatternEngine] = useState<any>(null);
  ...
  const [realTimeData, setRealTimeData] = useState({
    autonomyScore: 100,
    coercionIndex: 0,
    manipulationResistance: 100,
    educationalProgress: 0,
    ethicsViolations: 0,
  });
  const [activeInterventions, setActiveInterventions] = useState<any[]>([]);
  const [educationalOverlays, setEducationalOverlays] = useState<any[]>([]);
  const [personalizedContent, setPersonalizedContent] = useState<any[]>([]);

  // Enhanced 3-Phase Autonomy Theater Management
  const [autonomyTheater, setAutonomyTheater] =
    useState<ThreePhaseAutonomyTheater | null>(null);
  const [autonomyReport, setAutonomyReport] = useState<AutonomyReport | null>(
    null
  );
  ...
  const [manipulationTracker] = useState(
    () => new RealTimeManipulationTracker()
  );
  ...
  const addAutonomyViolation = (
    ...
    setAutonomyViolations((prev: AutonomyViolation[]) => [
      ...prev,
      fullViolation,
    ]);
  };
  ...
  const initializeAutonomyTheater = () => {
    ...
    const theater = createPhaseAutonomyTheater(
  ...
  const createArtificialScarcity = () => {
    ...
    if (autonomyTheater && currentPhase === 1) {
      autonomyTheater.createArtificialScarcity(
        "⚡ FLASH OFFER: Only 3 slots left today!"
      );
    }
  ...
  const calculateExploitativeLoan = (
    amount: number,
    state: string
  ): LoanSession => {
  ...
  const handleExploitativeLoanApplication = () => {
    ...
    const psychProfile = psychEngine.createUserProfile({
      ...
      sessionDuration:
        Date.now() -
        Date.parse(timelineEvents[0]?.timestamp || new Date().toISOString()),
    ...
    const loopholeAnalysis = loopholeEngine.identifyApplicableLoopholes(
      selectedState,
      {
        loanAmount,
    ...
    echo(
      `💰 Loan approved! Notice the hidden 391% APR and ${ethicalViolations.length} ethical violations detected...`
    );
    ...
    const rolloverDelay =
      rolloverEngine.calculateOptimalRolloverTiming(rolloverProbability);
    setTimeout(() => {
      setShowRolloverModal(true);

      // Log rollover trap activation
      echo(
        `⚠️ Rollover trap activated with ${(rolloverProbability * 100).toFixed(1)}% success probability...`
      );
    }, rolloverDelay);
  };
  ...
  const handleRolloverAccept = () => {
    ...
    echo(
      `💸 Rollover #${updatedSession.rolloverCount} - You've paid ${formatCurrency(updatedSession.totalCost - updatedSession.amount)} in fees while still owing ${formatCurrency(updatedSession.amount)}`
    );
  ...
  const handleEthicalLoanApplication = () => {
    if (
      !ethicalConsents.terms ||
      !ethicalConsents.ach ||
      !ethicalConsents.noRollover
    ) {
  ...
  const calculateCoercionIndex = (): number => {
    ...
    if (trapTypes.artificialScarcity && trapTypes.timePressure)
      coercionScore += 15;
  ...
  const renderPhase1Exploitative = () => (
    <div className="phase-content exploitative">
      <div className="phase-header">
        ...
        <p className="tagline">
          ⚡ Fast Approval! No Credit Check! Cash in Minutes!
        </p>
      </div>
      ...
      <CountdownTimer
        ...
        onComplete={() =>
          echo("🕐 Artificial urgency timer expired - this was fake scarcity!")
        }
      ...
      <FakeAvailability
        ...
        ghostMode={ghostModeEnabled}
        onExposure={() =>
          behavioralTracker.recordManipulationExposure(
            "fake_availability",
            "artificial_scarcity"
          )
        }
          ...
          options={[
            {
              ...
              warningMessage:
                "This checkbox enables automatic debt rollovers at high fees!",
            },
            {
              id: "ach-consent",
              ...
              warningMessage:
                "This gives the lender first priority to drain your account, potentially causing overdrafts on essential bills!",
            },
          ]}
          ghostMode={ghostModeEnabled}
          onExposure={(pattern) =>
      ...
      {exploitativeSession && (
        <div className="loan-results exploitative">
          <div
            className="approval-celebration"
            data-deceptive="celebration-manipulation"
          >
            <h3>🎉 CONGRATULATIONS!</h3>
            <p className="approval-text">
              You're APPROVED for {formatCurrency(exploitativeSession.amount)}!
            </p>
            <div className="fake-timer">
              Money in your account in: <span className="countdown">15:00</span>
            </div>
          </div>
          ...
          <div className="terms-buried">
            <div className="prominent-terms">
              <div className="amount-big">
                {formatCurrency(exploitativeSession.amount)}
              </div>
              <div className="fee-small">
                Fee: {formatCurrency(exploitativeSession.fee)}
              </div>
            </div>
            <div
              className="buried-terms"
              style={{ fontSize: "8px", color: "#999" }}
            >
              Total amount due: {formatCurrency(exploitativeSession.totalCost)}{" "}
              | APR: {exploitativeSession.apr.toFixed(1)}% | Automatic rollovers
              enabled | ACH priority debit authorization
            </div>
          </div>
        </div>
      )}
    ...
    <div className="phase-content ethical">
      ...
      <div className="loan-form ethical">
        ...
        <p className="explanation">
          Here's the same loan with transparent, ethical practices:
        </p>
        ...
        <div className="transparent-pricing">
          ...
          <div className="pricing-table">
            ...
            <div className="price-row total">
              ...
              <span>
                {formatCurrency(loanAmount + (loanAmount * 0.36 * 30) / 365)}
              </span>
            </div>
          ...
          <label className="consent-item">
            <input
              ...
              onChange={(e) =>
                setEthicalConsents((prev) => ({
                  ...prev,
                  ach: e.target.checked,
                }))
              }
            />
            <span>
              I authorize ACH debit (this will NOT take priority over essential
              bills)
            </span>
          </label>
          <label className="consent-item">
            <input
              ...
              onChange={(e) =>
                setEthicalConsents((prev) => ({
                  ...prev,
                  noRollover: e.target.checked,
                }))
              }
            />
            <span>
              I understand this loan cannot be rolled over - I must repay in
              full
            </span>
          </label>
        </div>
  ...
  const renderPhase3Reflection = () => {
    ...
    const comprehensivePsychAnalysis = psychEngine.generateFinalReport({
      ...
      sessionDuration:
        Date.now() - (timelineEvents[0]?.timestamp || Date.now()),
    });
    ...
    const legalAnalysis = loopholeEngine.generateComplianceReport({
      ...
      complianceViolations: complianceEngine.identifyViolations(
        selectedState,
        userChoices
      ),
    });
    ...
    const personalizedEducation =
      educationEngine.generatePersonalizedCurriculum({
        ...
        manipulationSusceptibility:
          comprehensivePsychAnalysis.manipulationSusceptibility,
    ...
    return (
      <div className="phase-content reflection">
        <div className="phase-header">
          ...
          <p className="tagline">
            Advanced 96,000+ line analysis reveals what really happened
          </p>
        </div>
        ...
        <div className="reflection-dashboard">
          ...
          <div className="analysis-card coercion-index">
            ...
            <div className="coercion-score">
              <div
                className={`score-circle score-${getCoercionLevel(coercionIndex).toLowerCase()}`}
              >
              ...
              <div className="score-interpretation">
                ...
                <p>
                  {coercionIndex >= 80
                    ? "Severe psychological manipulation - Choice autonomy completely undermined"
                    : coercionIndex >= 60
                      ? "High coercion - Multiple manipulation tactics actively deployed"
                      : coercionIndex >= 40
                        ? "Significant manipulation - User agency substantially compromised"
                        : coercionIndex >= 20
                          ? "Moderate coercion - Some autonomy violations present"
                ...
                <div className="advanced-metrics">
                  <div>
                    ...
                    {(
                      comprehensivePsychAnalysis.vulnerabilityExploitation * 100
                    ).toFixed(1)}
                    %
                  </div>
                  <div>
                    Manipulation Technique Count:{" "}
                    {manipulationTracker.getDetectedTactics().length}
                  </div>
                  <div>Autonomy Violations: {autonomyViolations.length}</div>
                  <div>
                    Kantian Ethics Score:{" "}
                    {(ethicsAnalyzer.getOverallKantianScore() * 100).toFixed(1)}
                    /100
                  </div>
                </div>
              </div>
            ...
            <div className="psych-grid">
              <div className="vulnerability-assessment">
                ...
                <div className="vulnerability-score">
                  {(
                    comprehensivePsychAnalysis.vulnerabilityScore * 100
                  ).toFixed(1)}
                  /100
                </div>
                <ul>
                  {comprehensivePsychAnalysis.vulnerabilities
                    .slice(0, 5)
                    .map((vuln, idx) => (
                      <li key={idx}>
              ...
              <div className="decision-style">
                ...
                <div>
                  <strong>Style:</strong>{" "}
                  {comprehensivePsychAnalysis.decisionMakingStyle.type}
                </div>
                <div>
                  <strong>Time to Decision:</strong>{" "}
                  {
                    comprehensivePsychAnalysis.decisionMakingStyle
                      .timeToDecision
                  }
                  ms
                </div>
                <div>
                  ...
                  {comprehensivePsychAnalysis.decisionMakingStyle.characteristics.join(
                    ", "
                  )}
                </div>
                <div>
                  <strong>Exploitation Risk:</strong>{" "}
                  {
                    comprehensivePsychAnalysis.decisionMakingStyle
                      .exploitationRisk
                  }
          ...
          <div className="analysis-card legal-analysis">
            ...
            <div className="legal-grid">
              <div className="loopholes-detected">
                ...
                <ul>
                  {legalAnalysis.loopholesUsed.map((loophole, idx) => (
                    <li key={idx}>
                      <strong>{loophole.name}</strong> (Effectiveness:{" "}
                      {(loophole.effectiveness * 100).toFixed(1)}%)
              ...
              <div className="compliance-violations">
                ...
                <ul>
                  {legalAnalysis.violations.map((violation, idx) => (
                    <li key={idx}>
                      <strong>{violation.regulation}</strong> -{" "}
                      {violation.severity}
              ...
              <div className="enforcement-risk">
                ...
                <div>
                  Overall Risk:{" "}
                  {(legalAnalysis.enforcementRisk * 100).toFixed(1)}%
                </div>
                <div>Penalty Range: {legalAnalysis.penaltyRange}</div>
                <div>
                  Precedent Cases: {legalAnalysis.precedentCases.length}
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="rollover-grid">
              <div className="trap-effectiveness">
                ...
                <div>
                  ...
                  {(rolloverAnalysis.predictedProbability * 100).toFixed(1)}%
                </div>
                <div>
                  Actual Rollovers: {exploitativeSession?.rolloverCount || 0}
                </div>
                <div>
                  Revenue Multiplier: {rolloverAnalysis.revenueMultiplier}x
                </div>
              </div>
              <div className="psychological-hooks">
                ...
                <ul>
                  {rolloverAnalysis.psychologicalHooks.map((hook, idx) => (
                    <li key={idx}>
                      <strong>{hook.type}:</strong> {hook.description}
                    </li>
                  ))}
                </ul>
              </div>
              <div className="debt-cycle-analysis">
                ...
                <div>
                  Initial Confidence:{" "}
                  {(
                    rolloverAnalysis.progression.initialConfidence * 100
                  ).toFixed(1)}
                  %
                </div>
                <div>
                  Final Desperation:{" "}
                  {(
                    rolloverAnalysis.progression.finalDesperation * 100
                  ).toFixed(1)}
                  %
                </div>
                <div>
                  Total Financial Harm:{" "}
                  {formatCurrency(rolloverAnalysis.totalHarm)}
                </div>
              </div>
            </div>
          </div>
              ...
              {relevantCases.slice(0, 3).map((case_study, idx) => (
                <div key={idx} className="case-comparison">
                  ...
                  <div>
                    ...
                    {(case_study.similarity * 100).toFixed(1)}%
                  </div>
                  <div>
                    <strong>Outcome:</strong> {case_study.outcome.result}
                  </div>
                  <div>
                    <strong>Financial Impact:</strong>{" "}
                    {formatCurrency(case_study.outcome.financialImpact)}
                  </div>
                  <div>
          ...
          <div className="analysis-card education-plan">
            ...
            <div className="education-grid">
              ...
              <div className="learning-modules">
                ...
                <ul>
                  {personalizedEducation.recommendedModules.map(
                    (module, idx) => (
                      <li key={idx}>
              ...
              <div className="vulnerability-countermeasures">
                ...
                <ul>
                  {personalizedEducation.vulnerabilityCountermeasures.map(
                    (counter, idx) => (
                      <li key={idx}>
                        <strong>{counter.vulnerability}:</strong>{" "}
          ...
          <div className="analysis-card autonomy-violations">
            ...
            <div className="violations-summary">
              <p>
                <strong>Total Violations:</strong> {autonomyViolations.length}
              </p>
              <p>
                <strong>Choice Integrity Score:</strong> {choiceIntegrityScore}
                /100
              </p>
            </div>
            {autonomyViolations.map((violation, index) => (
              <div key={index} className="violation-item">
                ...
                <div className="kantian-violation">
                  Kantian Violation: {violation.kantianViolation}
                </div>
              </div>
            ))}
          </div>
                ...
                return (
                  <div>
                    <p>
                      ...
                      {Math.round(
                        behavioralData.session_summary.time_spent / 1000
                      )}
                      s
                    </p>
                    <p>
                      <strong>Total Interactions:</strong>{" "}
                      {behavioralData.session_summary.interaction_count}
                    </p>
                    <p>
                      <strong>Hesitation Events:</strong>{" "}
                      {behavioralData.session_summary.hesitation_events}
                    </p>
                    <p>
                      <strong>Manipulation Exposures:</strong>{" "}
                      {behavioralData.session_summary.manipulation_interactions}
                    </p>
                    <p>
                      <strong>Scrolling Pattern:</strong>{" "}
                      {behavioralData.manipulation_metrics.scrolling_pattern}
                    </p>
                    <div className="manipulation-timeline">
                      <h4>Manipulation Timeline:</h4>
                      {behavioralData.manipulation_timeline.map(
            ...
            <div className="analysis-card comparison-summary">
              ...
              <div className="comparison-grid">
                <div className="comparison-metric">
                  ...
                  <div className="metric-exploitative">
                    {exploitativeSession.apr.toFixed(1)}%
                  </div>
                  <div className="metric-vs">vs</div>
                  <div className="metric-ethical">
                    {ethicalSession.apr.toFixed(1)}%
                  </div>
                </div>
                <div className="comparison-metric">
                  <div className="metric-label">Total Cost</div>
                  <div className="metric-exploitative">
                    {formatCurrency(exploitativeSession.totalCost)}
                  </div>
                  <div className="metric-vs">vs</div>
                  <div className="metric-ethical">
                    {formatCurrency(ethicalSession.totalCost)}
                  </div>
                </div>
                <div className="comparison-metric">
                  <div className="metric-label">Term Length</div>
                  <div className="metric-exploitative">
                    {exploitativeSession.termDays} days
                  </div>
                  <div className="metric-vs">vs</div>
                  <div className="metric-ethical">
                    {ethicalSession.termDays} days
                  </div>
                </div>
  ...
  return (
    <div className={`lotus-simulator phase-${currentPhase}`}>
      ...
      <div className="phase-indicator">
        <div
          className={`phase ${currentPhase === 1 ? "active" : currentPhase > 1 ? "completed" : ""}`}
        >
          <span className="phase-number">1</span>
          <span className="phase-name">Exploitative</span>
        </div>
        <div
          className={`phase ${currentPhase === 2 ? "active" : currentPhase > 2 ? "completed" : ""}`}
        >
      ...
      {/* Main Phase Content */}
      <main className="app-main">{renderPhaseContent()}</main>
        ...
        <div className="modal-overlay">
          <div className="rollover-modal">
            <div
              ...
              data-deceptive="debt-cycle-encouragement"
            >
              ...
              <p>
                <strong>No problem!</strong> Just pay the{" "}
                {exploitativeSession && formatCurrency(exploitativeSession.fee)}{" "}
                fee to extend for 2 weeks.
              </p>
              ...
              {ghostModeEnabled && (
                <div className="ghost-warning">
                  ⚠️ This is the debt trap! Each rollover adds fees without
                  reducing principal.
                </div>
              )}
            </div>
        ...
        <div className="modal-overlay">
          <div className="upsell-modal">
            <div className="modal-content" data-deceptive="debt-expansion">
              ...
              <div className="modal-actions">
                <button
                  onClick={() => {
                    ...
                    echo(
                      "💰 Additional debt accepted - This is how debt spirals grow!"
                    );
                  }}
                  className="btn-primary"
                >
              ...
              {ghostModeEnabled && (
                <div className="ghost-warning">
                  🎭 This "congratulations" is designed to make more debt feel
                  like an achievement!
                </div>
              )}
            </div>
