...
export const CountdownTimer: React.FC<CountdownTimerProps> = ({
  ...
  useEffect(() => {
    ...
    onManipulationDetected?.('artificial_urgency', urgencyLevel === 'high' ? 0.8 : 0.5);
...
export const FakeSocialProof: React.FC<FakeSocialProofProps> = ({
  ...
  useEffect(() => {
    onManipulationDetected?.('social_proof_manipulation', recentActivity ? 0.6 : 0.3);
...
export class BehavioralTracker {
  ...
  private mouseMovements: Array<{ x: number; y: number; timestamp: number; target: string }>;
  private hesitationEvents: Array<{ type: string; duration: number; timestamp: number; location: any }>;
  private scrollingPattern: Array<{ scrollY: number; timestamp: number; direction: string }>;
  private focusEvents: Array<{ type: string; timestamp: number }>;
  private manipulationExposure: Array<{ type: string; manipulation_technique: string; timestamp: number; element_details: any }>;
  ...
  private recordManipulationExposure(interactionType: string, element: Element): void {
  ...
  private calculateManipulationSusceptibility(): any {
    ...
    const manipulationRate = analysis.manipulation_interactions / Math.max(1, analysis.interaction_count);
