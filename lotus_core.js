  ...
  exportJson(filename = null) {
    const data = {
      ...
      consentData: {
        ...
        agreedToArbitration: this.agreedToArbitration,
      },
    };
...
export function calculateDebtTrapMetrics(session) {
  ...
  const totalCost = session.getCumulativeCost
    ? session.getCumulativeCost()
    : session.totalCost;
