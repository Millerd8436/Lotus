...
const DARK_PATTERNS: DarkPatternAnalysis[] = [
  {
    ...
    description: "Gradually revealing information to prevent informed decision-making",
    how_it_works: "Start with simple form, then add complexity and fees step by step",
    psychological_mechanism: "Sunk cost fallacy - users feel invested after starting",
    harm_level: 8,
    examples: [
  ...
  },
  {
    ...
    psychological_mechanism: "Anchoring bias - users fixate on initial low price",
    harm_level: 7,
    examples: [
...
const RESEARCH_DATA: ResearchData[] = [
  {
    study_title: "Payday Lending in America: Who Borrows, Where They Borrow, and Why",
    ...
    content_modules: [
      {
        ...
        duration: "10 minutes",
        key_concepts: ["Annual Percentage Rate", "Interest vs. Fees", "Term Impact"],
    ...
    switch (action) {
      case 'analyze_dark_patterns':
        ...
        return NextResponse.json({
          success: true,
          analysis: analysis,
          overall_harm_score: analysis.reduce((sum, p) => sum + p.harm_level, 0) / analysis.length,
          protection_summary: {
            immediate_actions: analysis.flatMap(p => p.protection_strategies.slice(0, 1)),
            long_term_strategies: analysis.flatMap(p => p.protection_strategies.slice(1)),
      ...
      case 'get_consumer_protections':
        ...
        return NextResponse.json({
          ...
          state_specific: stateProtections.find(s => s.state === state) || stateProtections[0]
        });
      ...
      case 'behavioral_analysis':
        const { user_actions: _user_actions, decision_points: _decision_points } = data;
        
        const cognitiveBiases: BehavioralAnalysis[] = [
          {
            cognitive_bias: "Anchoring Bias",
            description: "Tendency to rely heavily on first piece of information encountered",
            how_exploited: "Display attractive initial fee while hiding total cost",
            prevalence: "Very common in financial decisions",
            mitigation_strategies: ["Always compare total costs", "Set decision criteria beforehand", "Use comparison tools"]
          },
          {
            cognitive_bias: "Loss Aversion",
            description: "Tendency to prefer avoiding losses over acquiring equivalent gains",
            how_exploited: "Sunk cost fallacy - users continue after investing time",
            prevalence: "Universal psychological tendency",
            mitigation_strategies: ["Set time limits", "Use cooling-off periods", "Ignore sunk costs in decisions"]
          }
        ];
      ...
      case 'research_lookup':
        ...
        const relevantResearch = RESEARCH_DATA.filter(study => 
          study.study_title.toLowerCase().includes(research_topic.toLowerCase()) &&
        ...
        return NextResponse.json({
          ...
          meta_analysis: {
            ...
            average_sample_size: relevantResearch.reduce((sum, study) => sum + study.sample_size, 0) / relevantResearch.length,
            key_themes: [
  ...
  switch (type) {
    case 'dark_patterns_database':
      return NextResponse.json({
        ...
        harm_assessment: {
          ...
          criteria: ["Financial impact", "Psychological manipulation", "Difficulty to detect", "Frequency of use"]
        }
      });
    ...
    case 'research_statistics':
      return NextResponse.json({
        ...
        methodology_note: "Data compiled from peer-reviewed studies, government reports, and industry analysis"
      });
        ...
        learning_paths: [
          {
            ...
            duration: "2 hours",
            topics: ["APR calculation", "Dark pattern recognition", "Complaint process"]
          },
          {
            path_name: "Advanced Financial Literacy",
            duration: "4 hours",
            topics: ["Behavioral finance", "Alternative lending", "Policy analysis"]
          }
        ],
    ...
    case 'policy_recommendations':
      return NextResponse.json({
        ...
        evidence_base: "Recommendations based on research showing 80% debt trap rate and widespread consumer harm"
      });
    ...
    default:
      return NextResponse.json({
        ...
        api_info: {
          ...
          version: '1.0.0',
          description: 'Comprehensive analysis, education, and research about predatory lending',
          endpoints: {
            POST: ['analyze_dark_patterns', 'get_consumer_protections', 'get_educational_content', 'behavioral_analysis', 'research_lookup'],
            GET: ['dark_patterns_database', 'research_statistics', 'educational_catalog', 'policy_recommendations']
          }
        }
      });
  }
