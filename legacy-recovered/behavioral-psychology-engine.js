...
export class BehavioralPsychologyEngine {
  ...
  detectAnchoringBias(firstValue, userValue, context = "") {
    ...
    bias.detected = true;
    bias.severity = Math.max(
      bias.severity,
      this.calculateBiasSeverity(anchoringStrength),
    );
    ...
    this.echo.logAction("cognitive_bias_detected", {
      ...
      instance: instance,
      educational_note:
        "Anchoring bias: First number seen disproportionately influences subsequent judgments",
    });
  ...
  detectSunkCostFallacy(investmentAmount, additionalCost, userDecision) {
    ...
    const sunkCostRatio =
      investmentAmount / (investmentAmount + additionalCost);
    ...
    const instance = {
      ...
      severity:
        sunkCostRatio > 0.7 ? "high" : sunkCostRatio > 0.4 ? "medium" : "low",
    };
    ...
    if (userDecision === "continue" && sunkCostRatio > 0.3) {
      ...
      bias.severity = Math.max(
        bias.severity,
        this.calculateBiasSeverity(sunkCostRatio),
      );
      ...
      this.echo.logAction("cognitive_bias_detected", {
        ...
        instance: instance,
        educational_note:
          "Sunk cost fallacy: Continuing investment to justify past decisions rather than evaluating future value",
      });
    }
  ...
  detectPresentBias(immediateReward, futureReward, timeDelay, userChoice) {
    ...
    const discountedFutureValue =
      futureReward / Math.pow(1 + fairDiscountRate, timeDelay / 365);
    const rationalChoice =
      immediateReward > discountedFutureValue ? "immediate" : "future";
    ...
    const instance = {
      ...
      severity: this.calculatePresentBiasSeverity(
        immediateReward,
        futureReward,
        timeDelay,
      ),
    ...
    if (instance.biasDetected) {
      ...
      this.echo.logAction("cognitive_bias_detected", {
        ...
        instance: instance,
        educational_note:
          "Present bias: Overvaluing immediate rewards relative to future benefits",
      });
    }
  ...
  detectLossAversion(framingType, userResponse, expectedResponse) {
    ...
    const instance = {
      ...
      lossAversionDetected:
        framingType === "loss" && userResponse !== expectedResponse,
    ...
    if (instance.lossAversionDetected) {
      ...
      this.echo.logAction("cognitive_bias_detected", {
        ...
        instance: instance,
        educational_note:
          "Loss aversion: Fear of losses outweighs equivalent gains",
      });
    }
  ...
  analyzeDecisionSystem(decisionTime, complexity, pressure) {
    ...
    const system1Score = this.calculateSystem1Score(
      decisionTime,
      complexity,
      pressure,
    );
    ...
    const system2Score = this.calculateSystem2Score(
      decisionTime,
      complexity,
      pressure,
    );
    ...
    if (system1Score > system2Score) {
      ...
      this.echo.logAction("decision_system_analysis", {
        ...
        pressure: pressure,
        educational_note:
          "System 1 thinking: Fast, automatic, potentially exploitable by dark patterns",
      });
    } else {
      ...
      this.echo.logAction("decision_system_analysis", {
        ...
        pressure: pressure,
        educational_note:
          "System 2 thinking: Slow, deliberate, more resistant to manipulation",
      });
    }
    ...
    return {
      system1Score,
      system2Score,
      dominantSystem: system1Score > system2Score ? "System 1" : "System 2",
    };
  }
  ...
  trackMouseBehavior(event) {
    ...
    if (
      mouseData.speed > this.getStressThreshold() ||
      mouseData.acceleration > this.getAccelerationThreshold()
    ) {
  ...
  trackClickBehavior(event, phase) {
    ...
    if (phase === "mousedown") {
      ...
      if (hesitationTime > 100) {
        // More than 100ms indicates hesitation
  ...
  generateBiasProfile() {
    ...
    Object.entries(this.cognitiveBiases).forEach(([biasType, biasData]) => {
      profile[biasType] = {
        ...
        latestInstance:
          biasData.instances[biasData.instances.length - 1] || null,
  ...
  generateDecisionProfile() {
    ...
    return {
      system1Dominance:
        patterns.system1_activation /
        (patterns.system1_activation + patterns.system2_engagement),
  ...
  assessManipulationVulnerability() {
    const tacticsCount = Object.values(this.manipulationTactics).reduce(
      (sum, tactics) => sum + tactics.length,
      0,
    );
  ...
  generateStressProfile() {
    ...
    return {
      ...
      averageStressIntensity:
        stressResponses.reduce((sum, event) => sum + event.intensity, 0) /
  ...
  generatePersonalizedRecommendations() {
    ...
    if (biasProfile.anchoring.detected) {
      recommendations.push(
        "Practice questioning first numbers you see in financial decisions - they may be designed to influence you",
      );
    }

    if (biasProfile.sunk_cost.detected) {
      recommendations.push(
        "When evaluating continuation of financial commitments, focus on future costs/benefits, not past investments",
      );
    }

    if (decisionProfile.system1Dominance > 0.7) {
      recommendations.push(
        "You tend toward quick decisions - practice taking deliberate breaks before important financial choices",
      );
    }

    if (this.assessManipulationVulnerability().vulnerabilityScore > 50) {
      recommendations.push(
        "You showed sensitivity to manipulation tactics - be especially cautious of time pressure and emotional appeals",
      );
    }
  ...
  calculatePresentBiasSeverity(immediate, future, delay) {
    const impliedDiscountRate =
      (future - immediate) / immediate / (delay / 365);
  ...
  identifyPsychologicalTarget(tacticType) {
    const targets = {
      ...
      emotional_appeals:
        "Emotional decision-making, bypassing rational analysis",
  ...
  getManipulationEducationalNote(tacticType) {
    const notes = {
      time_pressure:
        "Time pressure activates System 1 thinking, reducing careful consideration",
      emotional_appeals:
        "Emotional appeals bypass rational analysis and exploit psychological vulnerabilities",
      social_pressure:
        "Social proof exploits our tendency to follow others, even without verification",
      artificial_scarcity:
        "Fake scarcity creates urgency and fear of loss to pressure quick decisions",
      complexity_overwhelm:
        "Information overload prevents careful analysis and increases reliance on shortcuts",
      false_reassurance:
        "Authority figures or credentials used to reduce skepticism and questioning",
      commitment_escalation:
        "Small commitments used to justify progressively larger ones",
    };
  ...
  calculateMouseAcceleration(event) {
  ...
  calculateStressIntensity(mouseData) {
    return mouseData.speed / 100;
  }
  calculateScrollDirection() {
    return window.lastScrollY > window.scrollY ? "up" : "down";
  }
  calculateScrollSpeed() {
    return Math.abs(window.scrollY - (window.lastScrollY || 0));
  }
  getCognitiveOverloadThreshold() {
    return 500;
  }
  calculateInputHesitation(event) {
    return Math.random() * 1000;
  }
  trackInputCorrections(event) {
    return Math.floor(Math.random() * 3);
  }
  getCurrentDecisionContext() {
    return "loan_application";
  }
  calculateVulnerabilityLevel(biasData) {
    return biasData.severity > 2
      ? "high"
      : biasData.severity > 1
        ? "medium"
        : "low";
  ...
  countEffectiveTactics() {
    return (
      Object.values(this.manipulationTactics).reduce(
        (sum, tactics) => sum + tactics.length,
        0,
      ) * 0.7
    );
  }
  identifyMostEffectiveTacticType() {
    return "time_pressure";
  }
  identifyResilienceFactors() {
    return ["slow_decision_making", "questioning_behavior"];
  }
  analyzeStressPattern() {
    return "gradually_increasing";
  }
  identifyPrimaryStressors() {
    return ["time_pressure", "complexity"];
  }
  identifyCopingMechanisms() {
    return ["careful_reading", "hesitation_before_clicking"];
  }
  identifyPrimaryDecisionChallenges() {
    return ["time_pressure", "complexity_overwhelm"];
  }
  identifyStrongestBiases() {
    return Object.entries(this.cognitiveBiases)
      .filter(([_, data]) => data.detected)
      .map(([bias, _]) => bias);
  }
  assessManipulationEffectiveness(tacticType, details) {
    return Math.random() * 0.8 + 0.2;
  }
  assessEthicalConcern(tacticType) {
    return ["time_pressure", "artificial_scarcity"].includes(tacticType)
      ? "high"
      : "medium";
  }
}
