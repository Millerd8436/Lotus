...
class EthicsEngine {
    ...
    static checkInformedConsent(session) {
        ...
        return {
            status: Object.values(pillars).every(p => p) ? 'COMPLIANT' : 'VIOLATION',
            pillars: pillars,
            violationCount: Object.values(pillars).filter(p => !p).length,
            complianceRate: (Object.values(pillars).filter(p => p).length / 5) * 100
        };
    }
    ...
    static getViolationContext(violationType) {
        const contexts = {
            "Violation_CapacityNotConfirmed": "Lender failed to verify borrower's capacity to understand loan terms and consequences",
            "Violation_DisclosureNotProvided": "Required loan terms, fees, and risks were not fully disclosed upfront",
            "Violation_ComprehensionNotMet": "Borrower did not demonstrate understanding of loan terms through assessment",
            "Violation_VoluntarinessQuestionable": "Evidence suggests coercion, pressure tactics, or compromised decision-making environment",
            "Violation_AuthorizationNotGiven": "Borrower did not provide clear, informed authorization for the loan transaction"
        };
    ...
    static generateEthicsReport(session) {
        ...
        return {
            ...
            // Overall Assessment
            overallEthics: {
                status: this.determineOverallEthicalStatus(consentAnalysis, kantianAnalysis),
                riskLevel: this.assessEthicalRisk(session),
                recommendations: this.generateEthicalRecommendations(consentAnalysis, kantianAnalysis)
            },
            
            // Educational Insights
            educationalInsights: this.generateEducationalInsights(session, consentAnalysis, kantianAnalysis)
        };
    }
    ...
    static determineOverallEthicalStatus(consentAnalysis, kantianAnalysis) {
        if (consentAnalysis.status === 'COMPLIANT' && kantianAnalysis.length === 0) {
            return 'ETHICAL';
        } else if (consentAnalysis.violationCount <= 1 && kantianAnalysis.length <= 1) {
            return 'QUESTIONABLE';
        } else {
            return 'UNETHICAL';
        }
    ...
    static assessEthicalRisk(session) {
        ...
        if (session.darkPatternsDetected && session.darkPatternsDetected.length > 3) riskScore += 30;
    ...
    static generateEthicalRecommendations(consentAnalysis, kantianAnalysis) {
        ...
        if (consentAnalysis.violationCount > 0) {
            recommendations.push("Improve informed consent procedures to ensure borrower understanding");
        }
        
        if (kantianAnalysis.length > 0) {
            recommendations.push("Revise lending practices to respect borrower dignity and autonomy");
        }
        
        if (consentAnalysis.complianceRate < 80) {
            recommendations.push("Implement comprehensive financial literacy education before loan approval");
        }
     ...
     */
    static generateEducationalInsights(session, consentAnalysis, kantianAnalysis) {
        const insights = [];
        
        insights.push("Informed consent requires five key elements: capacity, disclosure, comprehension, voluntariness, and authorization");
        
        if (session.darkPatternsDetected && session.darkPatternsDetected.length > 0) {
            insights.push("Dark patterns detected - these are design techniques that manipulate user behavior");
        }
        
        if (kantianAnalysis.length > 0) {
            insights.push("Kantian ethics requires treating people as ends in themselves, never merely as means");
        }
