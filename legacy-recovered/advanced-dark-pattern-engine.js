...
export class DarkPatternEngine {
    constructor(session, echo, config = {}) {
        ...
        this.patterns = {
            ...
            social_proof_manipulation: { severity: 'medium', detected: false, instances: [] },
            ...
            comparison_prevention: { severity: 'medium', detected: false, instances: [] }
        };
    ...
    implementTimePressurePattern(seconds, message = "⚡ Limited time offer!") {
        ...
        const pattern = {
            ...
            educationalNote: 'Artificial time pressure forces quick decisions and prevents careful consideration'
        };
    ...
    implementHiddenCosts(visibleCost, hiddenFees, totalCost) {
        const pattern = {
            ...
            educationalNote: 'Hidden fees are revealed only after commitment to manipulate price perception'
        };
    ...
    implementForcedContinuity(serviceName, autoRenewal = true, hardToCancel = true) {
        const pattern = {
            ...
            educationalNote: 'Auto-renewal with difficult cancellation creates ongoing revenue at user expense'
        };
    ...
    implementSocialProofManipulation(fakeTestimonials, fakeCounters) {
        const pattern = {
            ...
            educationalNote: 'Fabricated social proof exploits our tendency to follow others'
        };
    ...
    implementDripPricing(basePrice, addOns, finalPrice) {
        const pattern = {
            ...
            educationalNote: 'Gradually revealing higher costs after initial commitment'
        };
    ...
    getPatternIndicators(patternType) {
        const indicators = {
            false_urgency: ['countdown', 'limited time', 'hurry', 'expires', 'only.*left'],
            hidden_costs: ['additional fees', 'processing fee', 'service charge', 'hidden'],
            forced_continuity: ['auto-renew', 'automatically charged', 'continuous'],
            social_proof_manipulation: ['customers bought', 'people viewing', 'testimonial'],
    ...
    assessUserVulnerability() {
        ...
        const avgVulnerability = Object.values(vulnerability)
            .filter(v => typeof v === 'number')
            .reduce((sum, v) => sum + v, 0) / 3;
        
        vulnerability.overallRisk = avgVulnerability > 0.7 ? 'high' : 
                                   avgVulnerability > 0.4 ? 'medium' : 'low';
    ...
    generateEducationalInsights() {
        ...
        if (this.manipulationScore > 50) {
            insights.push("You were exposed to significant manipulation tactics designed to influence your decisions.");
        }
        
        if (this.detectedPatterns.has('false_urgency')) {
            insights.push("Time pressure tactics were used to prevent careful consideration of loan terms.");
        }
        
        if (this.detectedPatterns.has('hidden_costs')) {
            insights.push("Important cost information was concealed or de-emphasized to mislead you about the true price.");
        }
        
        if (this.coercionIndex > 0.5) {
            insights.push("The level of psychological coercion detected exceeds ethical lending standards.");
        }
        
        insights.push(`You encountered ${this.detectedPatterns.size} different types of dark patterns during this session.`);
    ...
    generateProtectionRecommendations() {
        ...
        if (this.detectedPatterns.has('false_urgency')) {
            recommendations.push("Always take time to consider financial decisions, regardless of artificial deadlines.");
        }
        
        if (this.detectedPatterns.has('hidden_costs')) {
            recommendations.push("Always ask for the total cost upfront and get it in writing before committing.");
        }
        
        if (this.detectedPatterns.has('forced_continuity')) {
            recommendations.push("Read all terms about automatic renewals and understand cancellation procedures before signing.");
        }
        
        if (this.manipulationScore > 30) {
            recommendations.push("Be especially cautious with lenders who use high-pressure or emotional tactics.");
        }
        
        recommendations.push("Compare offers from multiple lenders and consider credit union alternatives.");
        recommendations.push("Seek financial counseling if you're considering payday loans due to financial distress.");
    ...
    scanElementForPatterns(element) {
        ...
        if (text.match(/additional fees|processing fee|service charge/) || 
            classList.some(cls => cls.includes('hidden') || cls.includes('fine-print'))) {
    ...
    analyzeClickBehavior(event) {
        ...
        if (hesitationTime < 100) { // Very quick click might indicate pressure
    ...
    // Helper methods
    calculateTimeVulnerability() { return Math.random() * 0.5 + 0.3; }
    calculateSocialVulnerability() { return Math.random() * 0.5 + 0.3; }
    calculatePriceVulnerability() { return Math.random() * 0.5 + 0.3; }
    measureHesitation(element) { return Math.random() * 1000; }
    calculateScrollSpeed() { return Math.random() * 2000; }
